...
define(function(require) {
    ...
    var Slider = QuestionView.extend({
        ...
        setupQuestion: function() {
            ...
            if (this.model.get("_selectionData") !== undefined && this.model.get("_selectionData").length !== 0) {
                var boolItem = this.model.get("_selectionData");
                var binItem = "";
                for (var i = 0; i < boolItem.length ; i++) {
                     binItem += boolItem[i] ? 1 : 0;
                }
                var value = parseInt(bin2hex(binItem), 16);
                ...
                this.selectItem(i);
                this.model.set("_userAnswer", value);
                this.setupFeedback();
            } else {
                this.model.set({
                    _selectedItem: {}
        ...
        storeUserAnswer: function() {
            ...
            var binItem = hex2bin((this.model.get('_selectedItem').value).toString(16));
            var selectedData = [];
            for (var i = 0; i < binItem.length ; i++) {
                 selectedData.push((binItem[i] === "1"));
            }
            this.model.set("_selectionData", selectedData);
        },
